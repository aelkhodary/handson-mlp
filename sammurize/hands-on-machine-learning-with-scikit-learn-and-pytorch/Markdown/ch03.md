...
![A pixelated grayscale image of a handwritten digit resembling the number 5, from the MNIST dataset.](./Images/hmls_0301.png)
...
![Handwritten digits from the MNIST dataset, showcasing a variety of numbers used to train machine learning models.](./Images/hmls_0302.png)
...
![Illustrated confusion matrix with examples of true negatives, false positives, false negatives, and true positives, used to explain prediction performance.](./Images/hmls_0303.png)
...
It is often convenient to combine precision and recall into a single metric called the *F1 score*, especially when you need a single metric to compare two classifiers. The F1 score is the *harmonic mean* of precision and recall ([Equation 3-3](#equation_three_three)). Whereas the regular mean treats all values equally, the harmonic mean gives much more weight to low values. As a result, the classifier will only get a high F1 score if both recall and precision are high.
...
![Diagram illustrating the precision/recall trade-off with different decision thresholds; as the threshold increases, precision generally increases while recall decreases.](./Images/hmls_0304.png)
...
![Graph showing precision and recall curves as functions of decision threshold, illustrating their inverse relationship.](./Images/hmls_0305.png)
...
![Graph showing the precision-recall curve with a marked point where precision drops sharply as recall approaches 80%.](./Images/hmls_0306.png)
...
![ROC curve showing the true positive rate versus the false positive rate, with a highlighted point indicating a threshold for 90% precision and 48% recall.](./Images/hmls_0307.png)
...
The `precision_recall_curve()` function expects labels and scores for each instance, so we need to train the random forest classifier and make it assign a score to each instance. But the `RandomForestClassifier` class does not have a
`decision_function()` method, due to the way it works (we will cover this in [Chapter 6](ch06.html#ensembles_chapter)). Luckily, it has a `predict_proba()` method that returns estimated class probabilities for each instance, and we can just use the probability of the positive class as a score, so `precision_recall_curve()` will work.[4](ch03.html#id1369) We can call the `cross_val_​predict()` function to train the `RandomForestClassifier` using cross-validation and make it predict class probabilities for every image as
follows:
...
![Precision-recall curves compare the performance of RandomForestClassifier and SGDClassifier, with the RandomForest curve closer to the top-right corner, indicating superior performance.](./Images/hmls_0308.png)
...
As you can see in [Figure 3-8](#pr_curve_comparison_plot), the `RandomForestClassifier`’s PR curve looks much better than the `SGDClassifier`’s: it comes much closer to the top-right corner. Its F1 score and ROC AUC score are also significantly better:
...
![Confusion matrices displaying model predictions, with the left showing raw counts and the right normalized by row to percentage accuracy for each class.](./Images/hmls_0309.png)
...
![Confusion matrix diagrams showing error rates normalized by row on the left and by column on the right, highlighting common misclassification patterns.](./Images/hmls_0310.png)
...
![Confusion matrix showing images of handwritten digits 3 and 5, highlighting misclassifications by a simple classifier.](./Images/hmls_0311.png)
...
![A pixelated, noisy image of the number seven on the left and a clean version on the right, illustrating the effect of noise reduction.](./Images/hmls_0312.png)
...
![A grayscale pixelated image of the digit seven, demonstrating a cleaned-up version after applying a K-Neighbors Classifier.](./Images/hmls_0313.png)
...
[1](ch03.html#id1283-marker) By default Scikit-Learn caches downloaded datasets in a directory called *scikit\_learn\_data* in your home directory.
