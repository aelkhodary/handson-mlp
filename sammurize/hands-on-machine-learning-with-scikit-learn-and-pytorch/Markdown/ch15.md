...
![Diagram illustrating the Transformer model's process for translating English to Spanish, showing how the encoder generates contextual embeddings and the decoder predicts the next token in the translated sequence.](./Images/hmls_1501.png)
...
![Diagram showing the exponential growth of transformer models from 2018 to 2025, highlighting influential releases by organizations like OpenAI, Google, and Facebook, with parameter counts rising into trillions.](./Images/hmls_1502.png)
...
![Diagram of the original 2017 transformer architecture, showing the encoder and decoder layers with multi-head attention, feed forward, and positional encoding processes, illustrating how inputs are transformed into outputs.](./Images/hmls_1503.png)
...
* The scaling factor 1 / d k scales down the similarity scores to avoid saturating the softmax function, which would lead to tiny gradients. This factor was empirically shown to speed up and stabilize training.
...
![Diagram illustrating the architecture of a multi-head attention layer, highlighting the flow from linear transformations of values, keys, and queries through split layers, scaled dot-product attention, concatenation, and final linear transformation.](./Images/hmls_1504.png)
...
Now here’s an implementation of the decoder block. It closely matches the decoder block represented on the righthand side of [Figure 15-3](#transformer_diagram), with some additional
dropout:
...
![Diagram illustrating BERT pretraining showing input and target sequences for MLM and NSP tasks, highlighting masked, random, and unchanged tokens.](./Images/hmls_1505.png)
...
![Diagram illustrating the BERT model's pre-training on the left and fine-tuning on the right, highlighting different tasks achieved by modifying the classification head.](./Images/hmls_1506.png)
...
![Diagram showing BERT fine-tuning: left side illustrates sentence classification for sentiment analysis, right side shows token classification for named entity recognition (NER).](./Images/hmls_1507.png)
...
![Diagram illustrating the use of an encoder-only model, like BERT, to answer multiple-choice questions by generating scores for each option and applying a softmax layer to convert them to probabilities.](./Images/hmls_1508.png)
...
![Diagram illustrating DistilBERT pretraining using distillation losses from a teacher model (BERT) and a masked language modeling (MLM) loss.](./Images/hmls_1509.png)
...
![Diagram illustrating the reduction of a large embedding matrix into smaller embeddings combined with a linear layer to achieve dimensional projection.](./Images/hmls_1510.png)
...
![Diagram illustrating the replaced token detection (RTD) process, showing how the generator suggests possible replacements for a masked word, and the discriminator evaluates the sentence for accuracy.](./Images/hmls_1511.png)
...
![Diagram illustrating how a decoder-only model, like GPT, generates text one token at a time by predicting the next word in the sequence, such as continuing "Happy birthday" with "to you!".](./Images/hmls_1512.png)
...
![Diagram illustrating GPT-1 pretraining with next token prediction using softmax and fine-tuning for classification with a sigmoid function.](./Images/hmls_1513.png)
...
![Screenshot of the Hugging Face model page for Mistral-7B-v0.3 showing a notification requiring users to agree to share contact information to access the model.](./Images/hmls_1514.png)
...
![Interface showing the steps to create a Hugging Face access token with options for setting fine-grained permissions for repositories.](./Images/hmls_1515.png)
...
![Screenshot of a Colab interface showing a secrets manager, which includes options for configuring private environment variables, with fields for adding secret keys and managing their access.](./Images/hmls_1516.png)
...
![Diagram illustrating the process of building a chatbot, detailing stages from pretraining an untrained transformer, to fine-tuning a base model, and finally deploying the chatbot model with additional modules.](./Images/hmls_1517.png)
...
* *σ*(·) is the usual sigmoid function: 
...
:   Haystack lets you build systems that can understand complex questions, retrieve relevant information,
    and provide accurate answers, typically using RAG.
...
With that, you should have everything you need. For example, you could use LangChain to orchestrate a workflow that uses Ollama to run a local LLM, and Haystack to retrieve relevant information from a vector database.
Before we close this chapter, let’s take a brief look at some of the most influential encoder-decoder transformers.
