...
![Illustration of a biological neuron highlighting key components such as the cell body, dendrites, axon, telodendria, and synaptic terminals, demonstrating the neuron's structure and connection points within neural networks.](./Images/hmls_0901.png)
...
![Illustration of layered neuron networks in the human cerebral cortex, emphasizing the complex organization of biological neural networks.](./Images/hmls_0902.png)
...
![Diagram showing artificial neural networks performing logical computations, including AND, OR, and NOT operations with neurons labeled A, B, and C.](./Images/hmls_0903.png)
...
![Diagram of a Threshold Logic Unit (TLU) showing how inputs multiplied by weights are summed with a bias, and a step function is applied to determine the output.](./Images/hmls_0904.png)
...
![Diagram of a perceptron architecture with two input neurons connected to three output neurons in a fully connected layer, illustrating TLUs in the output layer.](./Images/hmls_0905.png)
...
* 𝐘 ^
   is the output matrix. It has one row per instance and one column per neuron.
...
In mathematics, the sum of a matrix and a vector is undefined. However, in data science, we allow “broadcasting”: adding a vector to a matrix means adding it to every row in the matrix. So,
**XW** + **b** first multiplies **X** by **W**—which results in a matrix with one row per instance and one column per output—then adds the vector **b** to every row of that matrix, which adds each bias term to the corresponding output, for every instance. Moreover, *ϕ* is then applied itemwise to each item in the resulting matrix.
...
* y ^ j 
   is the output of the *j*th output neuron for the current training instance.
...
![Diagram illustrating the XOR classification problem and an MLP using threshold logic units to solve it.](./Images/hmls_0906.png)

###### Figure 9-6. XOR classification problem and an MLP that solves it

![Diagram of a multilayer perceptron showing a feedforward neural network with two input neurons, a hidden layer of four neurons, and three output neurons.](./Images/hmls_0907.png)
...
![Diagram illustrating four activation functions—Heaviside, ReLU, Sigmoid, and Tanh—alongside their respective derivatives, highlighting nonlinearity essential for deep neural networks.](./Images/hmls_0908.png)
...
![A scatter plot shows the MLP regressor's predictions versus the targets, with most points clustering near the dashed red line indicating ideal predictions.](./Images/hmls_0909.png)
...
![Diagram illustrating a modern Multi-Layer Perceptron (MLP) architecture for classification, featuring input, hidden, and output layers with ReLU and softmax functions.](./Images/hmls_0910.png)
...
![Grid of the first four samples from each class in the Fashion MNIST dataset, showing various clothing and footwear items labeled by category.](./Images/hmls_0911.png)
...
[6](ch09.html#id2115-marker) Drawing of a cortical lamination by S. Ramon y Cajal (public domain). Reproduced from [*https://en.wikipedia.org/wiki/Cerebral\_cortex*](https://en.wikipedia.org/wiki/Cerebral_cortex).
